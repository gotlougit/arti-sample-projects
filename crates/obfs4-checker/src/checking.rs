...
async fn controlled_test_function(
    ...
    while counter < node_lines.len() {
        ...
        for _ in 0..MAX_CONNECTIONS {
            ...
            let bridge: BridgeConfigBuilder = rawbridgeline.parse().unwrap();
            let bridge_config = bridge.build().unwrap();
            let tor_client = common_tor_client.isolated_client();
            tasks.push(tokio::spawn(async move {
                let current_time = Utc::now();
                let formatted_time = current_time.format("%Y-%m-%dT%H:%M:%S%.9fZ").to_string();
                let (functional, error) = is_bridge_online(&bridge_config, &tor_client).await;
        ...
        for task in task_results {
            match task {
                Ok((bridgeline, functional, time, error)) => {
                    let res = BridgeResult {
                        functional,
                        last_tested: time,
                        error,
                    };
                    results.insert(bridgeline, res);
                }
                Err(e) => {
                    error!("{}", e.report());
                }
...
pub async fn main_test(
...
) -> Result<HashMap<String, BridgeResult>, Box<dyn Error>> {
    ...
    let bridge_results = controlled_test_function(&guard_lines, common_tor_client).await;
    Ok(bridge_results)
}
